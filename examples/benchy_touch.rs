...
fn main() -> ! {
    ...
    render_3d_benchy(delay, display, rtc, touch);
    unreachable!();
}
...
>(
    ...
    let mut benchy = K3dMesh::new(embed_stl!("examples/mesh/benchy.stl"));
    ...
    let mut last_touch_x: f32 = -1.0; //size.width / 2;
    ...
    loop {
        ...
        if let Some(ref mut touches) = touch.poll() {
            if let Some(TouchEvent::TouchDown(y, x, pressure)) = touches.pop() {
                if last_touch_x > 0.0f32 {
                    moving_parameter += (x as f32 - last_touch_x as f32) * 0.01f32;
                }
                last_touch_x = x as f32;
            }
        } else {
            last_touch_x = -1.0f32;
        }
